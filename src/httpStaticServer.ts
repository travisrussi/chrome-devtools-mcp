...
function cleanTmpDirectory(directory: string) {
  ...
  try {
    ...
    console.error(`Cleaned tmp directory: ${directory}`);
  } catch (err) {
    console.error(`Error cleaning tmp directory: ${err}`);
  }
}

export function startStaticHttpServer(): {server: http.Server; port: number} {
  const TMP_DIR = path.resolve(__dirname, '../tmp');
  const HTTP_PORT = process.env.CHROME_DEVTOOLS_MCP_HTTP_PORT
    ? parseInt(process.env.CHROME_DEVTOOLS_MCP_HTTP_PORT, 10)
    : 8080;
    ...
    if (req.url === '/tmp/' || req.url === '/tmp') {
      fs.readdir(TMP_DIR, (err, files) => {
        ...
        const links = files
          ...
          .map(
            f =>
              `<li><a href="/tmp/${encodeURIComponent(f)}">${f}</a></li>`,
          ) // encode for safety
          .join('\n');
  ...
  server.on('error', (err: NodeJS.ErrnoException) => {
    if (err.code === 'EADDRINUSE') {
      console.error(
        `Port ${HTTP_PORT} is already in use. Trying a random available port...`,
      );
      ...
      server.listen(0, () => {
        ...
        console.error(
          `Static file server running at http://localhost:${actualPort}/tmp/`,
        );
      });
    } else {
